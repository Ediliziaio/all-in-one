...
export default function MiaCamera() {
  ...
  return (
    <PageTransition className="p-6 space-y-6">
      <FadeIn><h1 className="font-heading text-2xl font-bold">La Mia Camera</h1></FadeIn>

      {/* Galleria */}
      <FadeIn delay={0.1}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {room.images.slice(0, 4).map((img, i) => (
            <img key={i} src={img} alt={`Foto ${i + 1}`} className={`rounded-xl object-cover w-full transition-transform duration-300 hover:scale-[1.03] ${i === 0 ? "col-span-2 row-span-2 h-64" : "h-32"}`} />
          ))}
        </div>
      </FadeIn>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardContent className="p-4 space-y-3">
                ...
                {["Giugno 2025", "Maggio 2025", "Aprile 2025"].map((m) => (
